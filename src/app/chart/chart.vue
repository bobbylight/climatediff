...
export default class Chart extends Vue {
    ...
    @Prop({ required: true })
    chartTitle: string; // Not private, used in template
    ...
    showErrors: boolean[] = []; // Not private, used in template

    private selectedUnits: UnitConfig = this.chartConfig.units[0];

    private tips: D3ToolTip[] = [];
    ...
    createChartAndShowErrors(data: Response<any>) {
        ...
        this.showErrors = this.errors.map(() => { return true; });
        this.createChart();
    }
    ...
    @Watch('data')
    onDataChanged(newValue: Response<any>) {
